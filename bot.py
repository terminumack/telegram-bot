...
FILTER_MIN_USD = 20 # <--- MONTO EN DÓLARES PARA EL FILTRO
...
def get_detailed_report_text():
    ...
    try:
        ...
        text += f"\n<i>El sistema opera con normalidad (V19).</i> ✅"
        return text
...
# ==============================================================================
#  BACKEND PRECIOS (ALGORITMO DINÁMICO CORREGIDO)
# ==============================================================================
def fetch_binance_price():
    ...
    payload = {
        ...
        "transAmount": str(dynamic_amount), # <--- USO DEL DINÁMICO
    ...
    try:
        ...
        # Fallback 1: Si no hay con filtro dinámico, intentamos sin merchant
        if not data.get("data"):
            del payload["publisherType"]
            response = requests.post(url, json=payload, headers=headers, timeout=10)
            data = response.json()
        
        # Fallback 2: Sin bancos específicos
        if not data.get("data"):
            payload["payTypes"] = ["PagoMovil"]
            response = requests.post(url, json=payload, headers=headers, timeout=10)
            data = response.json()
            
        # Fallback 3: Sin monto (Último recurso)
        if not data.get("data"):
